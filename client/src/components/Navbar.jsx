...
function Navbar() {
  ...
  return (
    ...
    transition={{duration:0.5}}
     className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-gray-600 border-b bg-gradient-to-r from-blue-100 via-indigo-200 to-purple-100
 relative transition-all ${location.pathname === "/"&&"bg-light"}`}>
        <Link to='/'>
         <motion.img
         whileHover={{scale:1.05}}
           src={assets.logo} alt="log"className='h-8' />
        </Link>
